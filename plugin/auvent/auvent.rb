...
#    2 liteaux 1 pente ( commenter l'indice 1 du menu) ou 3 liteaux 2 pentes(indice 0 et 1)
    def self.generate_couverture(cover_type_key, root,garage=nil,option=0)
      ...
      if liteaux.length < 2
        puts("abandon, nb. of battens < 2 / abandon,nb batten < 2 #{liteaux.length}")
        return
      end
    ...
    def self.profil_renfort_45(root,p)
      ...
      if(y1>y2)
        pts_monde = [
          Geom::Point3d.new(x1-sxy/2,y1-diag,z1),
          Geom::Point3d.new(x1-sxy/2,y1+diag,z1),
          Geom::Point3d.new(x2-sxy/2,y2,z2-diag),
          Geom::Point3d.new(x2-sxy/2,y2,z2+diag),
        ]
      elsif(y1<y2)
        pts_monde = [
        ...
        puts "case renfort_45 not provided for"
        return
      end
...
#*********************************Plans 2D**************************************
    ...
    def self.export_plan_a4_optimise
      ...
      return UI.messagebox("Aucune instance du composant.") if inst.nil?
    ...
    def self.generate_2d_views_from(entity)
      ...
      if entity.is_a?(Sketchup::Group)
        entites = entity.entities
        tr_entity = entity.transformation
      elsif entity.is_a?(Sketchup::ComponentInstance)
        entites = entity.definition.entities
        tr_entity = entity.transformation
      else
      ...
      mots_cles = ["CHEVRON", "RAFTER"]
      chevrons = UtilsAuvent.filtrer_groupes_par_mots_cles(entites, mots_cles)
      if chevrons.empty?
    ...
    def self.add_panneau_clins_3d(name,parent, origin,
      ...
      # Extrusion material
      # -----------------------------
      mat = UtilsAuvent.resolve_material(material_name,@materials)
      faces.each do |f|
        f.material = mat
        f.back_material = mat
      ...
      if(last_component_name!="ABRI_SUD")
        ...
        coupe_panneau_auto(clins_group,chevrons,material_name,1)
      end
      clins_group
    end
      ...
      if orientation == :horizontal
        ...
        if(delta_hauteur > 0)
          ...
          when :xplus, :xminus
            ...
            ossature_group = get_ossature_group()
            chevrons=get_chevrons_group(ossature_group)
            coupe_panneau_auto(lambris_group,chevrons,material_name)
          end
        else
          faces.each do |f|
            f.material = mat
    ...
    def self.extrude_panneau_polycarbonate(root, pts, profil, pas, material_name,name,id)
    ...
    def self.coupe_panneau_auto(lambris_group, chevrons,material_name,cas=0)
    ...
    def self.extrude_panneau(root, new_pts, thickness_i, material_name,name,id)
      gents = root.entities.add_group
      gents.name="[#{name}] #{id}"
      gents.set_attribute(name, "id", id.to_s)
    ...
    def self.piece_xy(root, p)
      ...
      option=p[9]
      face_ext = nil
      if(option==1.mm.to_f || option==2.mm.to_f || option==3.mm.to_f)
      ...
      #g.material=mat  #colore les coupes
      apply_material(g.entities,mat)
      return g
    end
    ...
    def self.piece_panneau(root, p)
      ...
      model = Sketchup.active_model
      if (type == 1.mm.to_f)           #polycarbonate 
        profil  = profil_polycarbonate_15mm_epais(thickness_i,largeur)
        extrude_panneau_polycarbonate(root, new_pts, profil, largeur, material_name,name,id)
      elsif (type == 2.mm.to_f)        #panneau plein
        if(option == 1.mm.to_f)   #angles arrondis de rayon  #voir profil de dessus avec les arrondis
          grp=profil_rectangulaire_extrude_dans_epaisseur(root,longueur,largeur,rayon,thickness_i,name,id)
          ...
          extrude_panneau(root, new_pts,thickness_i,material_name,name,id)
        end
        #else
    ...
    def self.profil_rectangulaire_extrude_dans_epaisseur(root,longueur, largeur, rayon,epaisseur,name,id)
      ents=root.entities
      # Groupe pour le profil
      grp = ents.add_group
      gents = grp.entities
      grp.name="[#{name}] #{id}"
      grp.set_attribute(name, "id", id.to_s)
      # Demi-dimensions
    ...
    def self.generer_goutiere(rive,root, layer_auvent,p)
      ...
      closer.find_faces
      apply_material(grp.entities,material_name)
      grp
    end
    ...
    def self.generer_rives(root, layer_auvent,p)
      ...
      entites = group.entities
      edges = entites.grep(Sketchup::Edge)
      mots_cles = ["LITEAU", "BATTEN"]
      liteaux = UtilsAuvent.filtrer_groupes_par_mots_cles(entites, mots_cles)
    ...
    def self.generer_rive_basse(liteau,parent, layer_auvent,p)
      ...
      # Matériau
      apply_material(group.entities,material_name)
      group.name = p[0]
      group.layer = parent.layer
      group
    end
    ...
    def self.generate_liteaux_sur_chevrons(parent,p)
      name=p[0]
      nb_liteaux=p[1].to_mm.to_i
      material_name=p[-1]
      entites = parent.entities
      if(nb_liteaux<2)
        new_p=[-1..9]
        new_p[0]=p[0]
    ...
    def self.generate_chevrons_sur_poutres(parent, p)
      ...
      if(nb_chevrons<2)
        ...
        mots_cles = ["POUTRE", "BEAM"]
        poutres = UtilsAuvent.filtrer_groupes_par_mots_cles(entites, mots_cles)
    ...
    def self.lire_fichier_ossature(path)
      ...
      # 2. Extraction et analyse de la première ligne
      #premiere_ligne = lignes.shift # Supprime et retourne la 1ère ligne
      #if premiere_ligne =~ /VersionLogiciel=(.+)/
      #  version = $1 # Récupère "5.0"
      #else
      #  version = "1.0" # Version par défaut si non spécifiée
      #  lignes.unshift(premiere_ligne) # Remet la ligne si ce n'était pas la version
      #end
      #puts "version du paramétrage :  #{version}"
      lignes.each do |ligne|
        ...
        values = raw_values.map do |v|
          v_string = v.to_s.strip
          if v_string.empty?
            nil # Retourne nil si la valeur entre deux points-virgules est vide
          elsif v_string =~ /\A-?\d+(?:\.\d+)?\z/
            v_string=UtilsAuvent.eval_expr(v_string, vars).mm
          else
            v_string.downcase.to_sym
          end
        end
    ...
    def self.trouver_chevrons(obj)
      ...
      if obj.is_a?(Sketchup::Group) || obj.is_a?(Sketchup::ComponentInstance)
        entites = obj.entities
        #entites.grep(Sketchup::Group).select { |g| g.name.to_s.upcase.include?("CHEVRON") }
        mots_cles = ["CHEVRON", "RAFTER"]
        resultat += UtilsAuvent.filtrer_groupes_par_mots_cles(entites, mots_cles)
        entites.grep(Sketchup::Group).each { |g| resultat += trouver_chevrons(g) }
      end
      #if obj.is_a?(Sketchup::Group) || obj.is_a?(Sketchup::ComponentInstance)
        #entites = obj.entities
        #resultat += entites.grep(Sketchup::Group).select { |g| g.name.to_s.upcase.include?("RAFTER") }
        #entites.grep(Sketchup::Group).each { |g| resultat += trouver_chevrons(g) }
      #end
      resultat
    end
    ...
    def self.delete_liteaux(entites)
      mots_cles = ["LITEAU", "BATTEN"]
      liteaux = UtilsAuvent.filtrer_groupes_par_mots_cles(entites, mots_cles)
      liteaux.each(&:erase!)
    end

    def self.delete_rive(entites)
      mots_cles = ["RIVES", "FASCIA"]
      rive = UtilsAuvent.filtrer_groupes_par_mots_cles(entites, mots_cles)
      rive.each(&:erase!)
    end
