...
def self.filtrer_groupes_par_mots_cles(entites, mots_cles)
  # On s'assure que tous les mots-clés sont en majuscules pour une comparaison insensible à la casse
  mots_cles_upcase = mots_cles.map { |mot| mot.to_s.upcase }
  
  # On filtre les groupes
  entites.grep(Sketchup::Group).select do |g|
    nom_groupe = g.name.to_s.upcase
    
    # .any? renvoie true dès qu'un des mots-clés est trouvé dans le nom
    mots_cles_upcase.any? { |mot| nom_groupe.include?(mot) }
  end
end
...
#***********************************************************************************************
    ...
    def self.resolve_material(material_name,materials)
      return nil if material_name.nil?
      mat=nil
      mat = materials[material_name]
      return mat if mat
      mats = Sketchup.active_model.materials
      # --- 1) Détection hexadécimale ---
      if material_name =~ /^#?0x?[0-9a-fA-F]{6}$/
        hex = material_name.gsub(/^#?0x?/,'')
        r = hex[0..1].to_i(16)
        g = hex[2..3].to_i(16)
        b = hex[4..5].to_i(16)
        # Nom unique pour éviter les collisions
        mat_name = "HEX_#{hex.upcase}"
        mat = mats[mat_name]
        unless mat
        ...
        couleurs_minuscules = Sketchup::Color.names.map(&:downcase)
        if couleurs_minuscules.include?(material_name.downcase)
          # Note : Sketchup::Color.new accepte aussi les chaînes en minuscules !
          mat= Sketchup::Color.new(material_name)
        else
          mat="red"
        end
      end
      if(mat==nil)         #materiel inconnu  unknown material
        mat="red"
      end
      mat
    end
...
#**************************************divers puts*********************************************************
    ...
    def self.eval_expr(expr, vars)
      ...
      # On garde une copie de la chaîne d'origine au cas où l'évaluation échouerait
      original_expr = expr.to_s.strip
      expr = original_expr.dup

      # Remplacer les variables par leurs valeurs
      vars.each { |k, v| expr = expr.gsub(/\b#{k}\b/, v.to_s) }
  
      # Convertir les degrés "15°" en radians
      expr = expr.gsub(/(\d+)\s*°/) {$1.to_f.radians.to_s}
      begin
        # Si après remplacement, expr contient du texte qui ne s'évalue pas (ex: "yminus"),
        # eval() lèvera une erreur capturée par le rescue.
        return eval(expr)
      rescue
        # IMPORTANT : Au lieu de retourner 0, on retourne la chaîne nettoyée d'origine.
       # Ainsi, "yminus", "vertical" et "MAT=0xff00ff" restent intacts pour la suite !
        return original_expr
     end
    end

    def self.eval_expr_correction(expr, vars)
      return expr if expr.is_a?(Numeric)
      ...
      # Convertir les degrés "15°" en radians
      expr = expr.gsub(/(\d+)\s*°/) { $1.to_f.radians.to_s }    #* Math::PI / 180.0
      begin
        return eval(expr)
      rescue
